...
public class Apriori extends AbstractAssociator implements OptionHandler,
  ...
  public String  toJSON()
  {
      ...
      JSONObject[] rule = new JSONObject[m_allTheRules[0].size()] ;
      for (int i = 0; i < m_allTheRules[0].size(); i++) {
        rule[i] = new JSONObject() ;
        
        String premise     = ((AprioriItemSet) m_allTheRules[0].get(i)).toString( m_instances ) ;
        String consequence = ((AprioriItemSet) m_allTheRules[1].get(i)).toString( m_instances ) ;

        rule[i].put("then" , consequence ) ;
        rule[i].put("if"   , premise     ) ;
